function initUploadFile(fileId, clickId, showId,uploadUrl,resultId,success) {
	this.fileId = fileId;
	this.showId = showId;
...
//	            计算每块瓦片的宽和高
	  ...
	  function upload(basestr, type, $li,name) {
	    ...
	    layer.open({content:"正在上传图片",skin: 'msg',time: 2});
	    xhr.onreadystatechange = function() {
	      if (xhr.readyState == 4 && xhr.status == 200) {
	        ...
	        //layer.open();
	        console.log(text + '：' + jsonData.body.path);
	        clearInterval(loop);
	        //当收到该消息时上传完毕
	        ...
	        $("#"+resultId).val(jsonData.body.path);
	        if(success){
	        	success(jsonData);
	        }
	        //$("#"+showId).append('<a href="' + imagedata.path + '">' + imagedata.name + '（' + imagedata.size + '）<img src="' + imagedata.path + '" /></a>');
	      }
	    };
	    //数据发送进度，前50%展示该进度
